...
module.exports = function(cfg){
  ...
  return function(req, res, next){
     ...
     */
    _listAllKeys(req.s3Bucket,req.s3Region,'.',staticFilesPath, function (err, files) {
      if (err){
        return common.error(req, res, next, 404, 'No album found', err);
      }
      content=allKeys[0];
      for (var i=1; i<allKeys.length; i++){
        content.concat(allKeys[i]);
      }
      console.log(content);
      ...
      data.albums = _getAlbums(content, staticFilesPath, pathFromReq);
      data.photos = _getPhotos(content, staticFilesPath, pathFromReq);
...
function _listAllKeys(s3Bucket,s3Region,marker,prefix, cb)
{
  aws.config.region = s3Region;
  s3.listObjects({Bucket: s3Bucket, Marker: marker, Prefix: prefix}, function(err, data){
    //console.log(data.Contents);
    allKeys.push(data.Contents);

    if(data.IsTruncated)
      _listAllKeys(s3Bucket,s3Region,data.NextMarker,prefix, cb);
    else
      cb();
  });
}
